...
# shift OPTIND --1 to iterate backwards through options
shift $(($OPTIND - 1))
...
# Ascertain if we have the lockfile utility
if [ -z "$(which lockfile | grep -v '^no ')" ] ; then
  echo "$0 failed: 'lockfile' utility not found in PATH." >&2
  exit 1
fi
# check if passed action is lock/unlock, proceeding accordingly
if [ "$action" = "lock" ] ; then
